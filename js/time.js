...
// --- ส่วนการทำงานหลัก (รอให้หน้าเว็บ HTML โหลดเสร็จก่อนค่อยเริ่มทำงาน) ---
window.addEventListener('DOMContentLoaded', () => {
    
    // 1. จัดการวันที่และเลขที่เอกสาร
    ...
    // 2. ระบบซิงค์ข้อความ พิมพ์แก้ซ้าย -> ขวาเปลี่ยน (ผูกการรับฟังไว้ก่อน)
    const syncInputs = [
    ...
    // 3. บังคับอัปเดตข้อมูลทุกช่องจากซ้ายไปขวาทันที (ทำหลังจากผูกคำสั่งในข้อ 2 เสร็จแล้ว)
    syncInputs.forEach(item => {
        const inputEl = document.getElementById(`in-${item.id}`);
        if (inputEl) {
